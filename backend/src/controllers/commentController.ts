...
export const getAllComments = async(req:Request<CommentParams>,res:Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to get comments, something broken in your code"});
  }
};
...
export const createComment = async(req:Request,res:Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to create comment, something broken in your code!"});
  }
}
...
export const deleteComment = async (req:Request,res:Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to comment product, something broken in your code!"});
  }
};